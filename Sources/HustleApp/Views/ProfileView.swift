        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 16) {
                Circle()
                    ...
                    .overlay(Text(session.currentUser?.fullName.initials ?? "HU").font(.title.bold()).foregroundStyle(.white))
                ...
                HStack {
                    VStack(alignment: .leading) {
                        Text("Gross earnings")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                        Text(snapshot.grossEarnings, format: .currency(code: "USD"))
                            .font(.headline)
                    }
                    Spacer()
                    VStack(alignment: .leading) {
                        Text("Net earnings")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                        Text(snapshot.netEarnings, format: .currency(code: "USD"))
                            .font(.headline)
                    }
                    Spacer()
                    VStack(alignment: .leading) {
                        Text("Growth")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                        Text(analyticsViewModel.weeklyGrowth, format: .percent.precision(.fractionLength(1)))
                            .font(.headline)
                    }
                }
            }
    ...
    private var analyticsHighlights: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
                ...
                MetricCard(title: "On-time delivery", valueText: (analyticsViewModel.currentSnapshot?.onTimeDeliveryRate ?? 0).formatted(.percent.precision(.fractionLength(1))), trend: "+5% vs last week")
